...
### Issue 7: 
用户应当也可以导出当前schedule的md源文件到指定位置，且可以将其转换为txt格式（以及简单的格式转变供txt文件使用）
对于所有导出方式，应当不仅仅只自定义文件名，应当也可以定义文件放到哪个文件夹里

### Issue 8:
程序应当也支持根据tag分类，排序，搜索，指定tag等 （有指令显示现在有的所有tag，且可以根据tag进行筛选）

### Issue 9:
doctor指令的作用应当更加广泛，如其应该检测运行程序所需的所有dependency是否齐全，以及告知用户如何补全（用户可选择性下载）
以及检查当前环境，确保程序运行所需的所有内容都齐全

### Issue 10
添加一个简介指令，如tm intro，介绍这个项目在干什么，作者，日期版本等等所有的信息
---
